...
router.post('/sessions', async (req,res,next)=>{ try { const body=validate(createSessionSchema, req.body); const { data: st, error } = await supabaseAdmin.from('session_types').select('*').eq('id', body.sessionTypeId).single(); if (error || !st) throw new HttpError(404,'Session type not found'); const start=new Date(body.start); const end=new Date(start.getTime() + (body.durationMins ?? st.default_duration_mins)*60*1000); res.json({ ok:true, data: await sessionService.createSession({ sessionTypeId: body.sessionTypeId, tokenTypeId: st.token_type_id, coachUserId: body.coachId, locationId: body.locationId ?? null, startAt: start.toISOString(), endAt: end.toISOString(), capacity: body.capacity ?? st.default_capacity }) }); } catch(e){ next(e);} });
...
router.patch('/sessions/:sessionId/coach', async (req,res,next)=>{ try { const body=validate(setCoachSchema, req.body); res.json({ ok:true, data: await sessionService.setCoach(req.params.sessionId, body.newCoachId) }); } catch(e){ next(e);} });
